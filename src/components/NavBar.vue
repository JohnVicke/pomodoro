<template>
  <div class="nav">
    <v-container v-if="toggleMenu" class="d-flex flex-column justify-center menu">
      <p>content</p>
      <p>this is lit</p>
      <p>home</p>
    </v-container>
  ...
  cursor: pointer;
        "
          @click="$router.push({ path: '/' })"
          :class="`mr-4 ${$route.path === '/' ? 'p-active' : ''}`"
        >
          home
        </p>
        <p
  ...
  cursor: pointer;
        "
          :class="`ml-4 ${$route.path === '/pomodoro' ? 'p-active' : ''}`"
        >
          timer
        </p>
      </div>
...
export default class NavBar extends Vue {
  ...
  @Emit()
  getLine2ClassName() {
    return this.toggleMenu ? "line2 open" : "line2";
  }
  @Emit()
  getLine1ClassName() {
    return this.toggleMenu ? "line1 open" : "line1";
  }
  onClick() {
